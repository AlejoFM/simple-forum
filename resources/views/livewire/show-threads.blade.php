<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex gap-10 py-12">
    <div class="w-64">
        ...
        <ul>
            @foreach($category as $categories)
                <li class="mb-2">
                    <a href="#" wire:click.prevent="filter('{{ $categories->id }}')" class="p-2 rounded-md flex bg-slate-800 items-center gap-2 text-white/60 hover:text-white font-semibold text-xs capitalize">
                        <span class="w-2 h-2 rounded-full" style="background-color: {{ $categories->color }};"></span>
                        {{ $categories->name }}
                    </a>
                </li>
            @endforeach
            ...
            <li>
                <a href="#" wire:click.prevent="filter('')" class="p-2 rounded-md flex bg-slate-800 items-center gap-2 text-white/60 hover:text-white font-semibold text-xs">
    ...
    <div class="w-full">
        <div>
            <form>
                <input type="text" placeholder="// ..." class="mb-4 bg-slate-800 border-0 rounded-md w-1/3 p-3 text-white/60 text-xs" wire:model="search">
            </form>
        </div>
        <!-- formulario -->
